                        ...
                        double *east, double *north)
{
    ...
    *east  = dlon * mv_geo_m_per_deg_lon(mid);
    *north = (lat2 - lat1) * mv_geo_m_per_deg_lat(mid);
}
...
{
    if (!mv_geo_valid(lat1, lon1) || !mv_geo_valid(lat2, lon2))
        return -1.0;
...
void mv_geo_proj_init(MvGeoProj *p, double lat0, double lon0)
{
    p->lat0  = lat0;
    p->lon0  = lon0;
    p->m_lat = mv_geo_m_per_deg_lat(lat0);
    p->m_lon = mv_geo_m_per_deg_lon(lat0);
    if (p->m_lon < 1.0)
...
void mv_geo_bounds_extent_m(const MvGeoBounds *b, double *w_m, double *h_m)
{
    ...
    *w_m = (b->max_lon - b->min_lon) * mv_geo_m_per_deg_lon(mid);
    *h_m = (b->max_lat - b->min_lat) * mv_geo_m_per_deg_lat(mid);
}
