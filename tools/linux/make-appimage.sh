...
#   -> dist/magview-<version>-x86_64.AppImage
#
...
VERSION="$(sed -n 's/.*MAGVIEW_VERSION "\([^"]*\)".*/\1/p' "$ROOT/src/mv_version.h")"
OUT="$ROOT/dist/magview-$VERSION-x86_64.AppImage"
...
APPDIR="$ROOT/dist/magview.AppDir"
rm -rf "$APPDIR"
mkdir -p "$APPDIR/usr/bin" "$APPDIR/usr/lib" \
         "$APPDIR/usr/share/applications" \
         "$APPDIR/usr/share/icons/hicolor/256x256/apps" \
         "$APPDIR/usr/share/magview" \
         "$APPDIR/usr/share/doc/magview"

cp "$ROOT/magview"                "$APPDIR/usr/bin/"
cp "$HERE/AppRun"                 "$APPDIR/AppRun"
cp "$HERE/magview.desktop"        "$APPDIR/magview.desktop"
cp "$HERE/magview.desktop"        "$APPDIR/usr/share/applications/"
cp "$HERE/magview.png"            "$APPDIR/magview.png"
cp "$HERE/magview.png"            "$APPDIR/usr/share/icons/hicolor/256x256/apps/"
cp "$ROOT/LICENSE"                "$APPDIR/usr/share/doc/magview/"
cp "$ROOT/docs/HELP.md"           "$APPDIR/usr/share/doc/magview/"
...
cp "$HERE/font/DejaVuSans.ttf"      "$APPDIR/usr/share/magview/"
cp "$HERE/font/DejaVu-LICENSE.txt"  "$APPDIR/usr/share/doc/magview/"
